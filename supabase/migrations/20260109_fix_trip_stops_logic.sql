-- Migration to FIX trip generation by including both Pickup AND Dropoff stops for every child.
-- Defines new versions of generate_go_trips and generate_return_trips.
...
BEGIN
  -- Temp table with School Location columns added
  CREATE TEMP TABLE IF NOT EXISTS _temp_go_stops_v2 (
    booking_id UUID,
    child_id UUID,
    pickup_lat FLOAT,
    pickup_lng FLOAT,
  ...
  LOOP
    ...
    DELETE FROM _temp_go_stops_v2 WHERE TRUE;

    -- Gather children AND school locations
    INSERT INTO _temp_go_stops_v2 (booking_id, child_id, pickup_lat, pickup_lng, school_lat, school_lng)
    SELECT b.id, bc.child_id, c.pickup_lat, c.pickup_lng, b.school_lat, b.school_lng
    ...
    SELECT EXISTS (SELECT 1 FROM _temp_go_stops_v2) INTO has_children;
    ...
    IF has_children THEN
        ...
        -- 1. Insert PICKUPS (Home)
        FOR child_rec IN SELECT * FROM _temp_go_stops_v2 LOOP
        ...
        -- 2. Insert DROPOFFS (School)
        -- Ordered by school location to group drops (optional optimization)
        FOR child_rec IN SELECT * FROM _temp_go_stops_v2 ORDER BY school_lat, school_lng LOOP
            INSERT INTO public.route_stops (
                trip_id, booking_id, child_id, stop_type, sequence_order, 
                location_lat, location_lng, status
            )
...
DECLARE
  ...
  target_day_name TEXT;
BEGIN
  CREATE TEMP TABLE IF NOT EXISTS _temp_return_stops_v2 (
    booking_id UUID,
    child_id UUID,
    pickup_lat FLOAT,
    pickup_lng FLOAT,
  ...
  LOOP
    ...
    DELETE FROM _temp_return_stops_v2 WHERE TRUE;

    INSERT INTO _temp_return_stops_v2 (booking_id, child_id, pickup_lat, pickup_lng, school_lat, school_lng)
    SELECT b.id, bc.child_id, c.pickup_lat, c.pickup_lng, b.school_lat, b.school_lng
      FROM public.bookings b
      JOIN public.booking_children bc ON b.id = bc.booking_id
      JOIN public.children c ON bc.child_id = c.id
    ...
    SELECT EXISTS (SELECT 1 FROM _temp_return_stops_v2) INTO has_children;
    ...
    IF has_children THEN
        ...
        -- 1. Insert PICKUPS (School)
        FOR child_rec IN SELECT * FROM _temp_return_stops_v2 ORDER BY school_lat, school_lng LOOP
            INSERT INTO public.route_stops (
                trip_id, booking_id, child_id, stop_type, sequence_order, 
                location_lat, location_lng, status
            )
        ...
        -- 2. Insert DROPOFFS (Home)
        FOR child_rec IN SELECT * FROM _temp_return_stops_v2 LOOP
            INSERT INTO public.route_stops (
                trip_id, booking_id, child_id, stop_type, sequence_order, 
                location_lat, location_lng, status
            )
