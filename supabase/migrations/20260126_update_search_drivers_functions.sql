-- FIX search_drivers functions after moving is_online to drivers table

-- 1. Update search_drivers (PLPGSQL)
...
BEGIN
    ...
    SELECT
        ...
        d.bio,
        u.phone,
        -- MODIFIED: Use d.is_online instead of l.is_online
...
-- 2. Update search_drivers_for_parent_v2
...
BEGIN
    ...
    WHERE d.is_active = TRUE
      ...
      -- MODIFIED: Use d.is_online
      AND (filter_online_only IS FALSE OR COALESCE(d.is_online, false) = TRUE)
      AND (
    ...
    ORDER BY
       -- MODIFIED: Use d.is_online
       COALESCE(d.is_online, false) DESC,
       CASE
           WHEN parent_geo IS NOT NULL AND u.location_geo IS NOT NULL
