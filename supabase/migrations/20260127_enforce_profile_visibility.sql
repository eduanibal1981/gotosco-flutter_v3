-- FIX SEARCH VISIBILITY MIGRATION
-- Purpose: Enforce d.is_profile_online = true in search results, regardless of filter_online_only
-- This ensures that 'hidden' drivers (Ads offline) are never shown to parents.

-- A. Update search_drivers to strictly enforce is_profile_online = true
...
BEGIN
    ...
    WHERE
        ...
        -- STRICT ENFORCEMENT OF ONLINE STATUS
        AND COALESCE(d.is_profile_online, false) = TRUE;
END;
$$;

-- B. Update search_drivers_for_parent_v2 to strictly enforce is_profile_online = true
