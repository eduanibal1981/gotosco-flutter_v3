...
import 'package:gotosco_v3/features/driver/profile/domain/models/area_model.dart';
...
import 'package:gotosco_v3/features/driver/profile/domain/models/school_model.dart';
...
void main() {
  testWidgets('CoverageSummaryContent displays city name in chips', (
    ...
    // 1. Setup Data
    final city = CityModel(id: 'c1', name: 'Muscat');
    // We need at least 2 areas in the city so that "All Areas" logic isn't triggered
    final area1 = AreaModel(id: 'a1', cityId: 'c1', name: 'Bawsher');
    final area2 = AreaModel(id: 'a2', cityId: 'c1', name: 'Seeb');

    // Similarly for schools
    final school1 = SchoolModel(id: 's1', cityId: 'c1', name: 'British School');
    final school2 = SchoolModel(
      id: 's2',
      cityId: 'c1',
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          coverageAllAreasProvider.overrideWith(
            (ref) => Future.value([area1, area2]),
          ),
          coverageAllSchoolsProvider.overrideWith(
            (ref) => Future.value([school1, school2]),
          ),
          // Only select the first one of each
          driverCoverageAreaIdsProvider.overrideWith(
            (ref) => Future.value({'a1'}),
          ),
          driverCoverageSchoolIdsProvider.overrideWith(
            (ref) => Future.value({'s1'}),
          ),
        ],
        child: const MaterialApp(
          home: Scaffold(body: CoverageSummaryContent()),
        ),
      ),
    );
